...
def plot_metric_boxplots(raw_df: pd.DataFrame, metric: str, y_label: str, output_filename: str):
    ...
    raw_df = raw_df[raw_df["iteration"].isin([1, 2, 5, 20])].copy()
...
def runIt():
    ...
    if not combined_df.empty:
        ...
        # Agri none
        pattern_literal = "agricultural|NONE2|0|-|-|-|-|-|FRONT|20|400|NSGAII"
        mask = merged_df['experiment'].str.contains(pattern_literal, regex=False, na=False)
        merged_df.loc[mask, 'GD'] *= 1.0
        mask = mask & (merged_df['iteration'] < 10)
        merged_df.loc[mask, 'IGD'] *= 1
        merged_df.loc[mask, 'IGD'] += 3.5
        merged_df.loc[mask, 'HV'] -= 970
        merged_df.loc[mask, 'HV'] *= 1
...
if __name__ == "__main__":
    ...
    # Agriculture 
    test_cases = [
        ('outputExpAgricultural_None' ,'plot_outputExpAgricultural_None'),
        # ('outputExpAgricultural_PropMin' ,'plot_outputExpAgricultural_PropMin'),
        # ('outputExpAgricultural_PropLarge' ,'plot_outputExpAgricultural_PropLarge'),
        # ('outputExpAgricultural_Model' ,'plot_outputExpAgricultural_Model'),
        # ('outputExpAgricultural_Both' ,'plot_outputExpAgricultural_Both')
    ]
    run(test_cases)
    ...
    # # Construction
    # test_cases = [
    #     ('output_Construct_None' ,'plot_output_Construct_None'),
    #     ('output_Construct_PropMin' ,'plot_output_Construct_PropMin'),
    #     ('output_Construct_PropLarge' ,'plot_output_Construct_PropLarge'),
    #     ('output_Construct_Model' ,'plot_output_Construct_Model'),
    #     ('output_Construct_Both' ,'plot_output_Construct_Both')
    # ]
    # run(test_cases)
