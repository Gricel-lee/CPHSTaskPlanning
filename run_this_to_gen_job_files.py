...
python3 ./src/run.py --csv "input/experimentsAgricultural_iter.csv" --model "input/evomodel/AG/agricultural-humanNonLinear.pm" --properties "input/evomodel/AG/agricultural.pctl" --output "<REPLACE_OUTPUT_FOLDER_AND_EXPERIMENT_SUBFOLDER>" --optimisation "input/optimisationList_agricultural.txt" --changes "<REPLACE_CHANGES_FILE>"
'''
...
for num in numbering_experiments:
    ...
    with open("jobscript_" + name + ".job", "w") as f:
        ...
        s_new = s_new.replace("<REPLACE_OUTPUT_FOLDER_AND_EXPERIMENT_SUBFOLDER>", f"output{name_job_file}/agricultural{num}")
